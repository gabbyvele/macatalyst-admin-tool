        ...
        <div class="row">
            <div class="col-12">
                ...
                if (isset($_GET['news-id'])) {
                    $newsID = preg_replace("#[^0-9]#", "", $_GET["news-id"]);
                }

                if ($newsID != -1 && $newsID != "") {

                    $sqliQuery = $selectNewsArticleQuery . $newsID;
                    ...
                    if ($news = $res->fetch_assoc()) {
                        ...
                        $author = $news['Author'];
                        $date = date_format(date_create($news['Date']), "d F Y");
                        $industry = $news['Industry'];
                        $image = $news['Image'];
                        $content = $news['Content'];
                        $views = $news['Views'];
                        $createdBy = $news['created_by_user'];

                        $breakingNews;
                        if ($news['Breaking_news'] == 1) {
                            $breakingNews = "<input type=\"checkbox\" placeholder=\"Company\" id=\"breaking-news\" checked>";
                        } else {
                            $breakingNews = "<input type=\"checkbox\" placeholder=\"Company\" id=\"breaking-news\">";
                        }

                        echo "
                            <h1>Edit News</h1>
                            <div class=\"container-fluid content\">
                                Headline: <input type=\"text\" placeholder=\"Headline\" class=\"form-control\" id=\"news-headline\" value=\"$headline\">
                                <br>
                                Breaking News: $breakingNews
                                (Check this box if this is is breaking news)
                                &nbsp
                                <br>
                                Author: <input type=\"text\" placeholder=\"Author\" class=\"form-control\" id=\"news-author\" value=\"$author\">
                                News Industry:
                        ...
                        while ($industryTypes = $industries->fetch_assoc()) {
                            $industryID = $industryTypes["id"];
                            $name = $industryTypes["name"];
                            if ($industryID == $industry) {
                                echo "<option value=\"$industryID\" selected>$name</option>";
                            } else {
                                echo "<option value=\"$industryID\">$name</option>";
                            }
                        }
                        ...
                        echo "  
                                ...
                                <div class='input-group'>
                                    <label class=\"btn btn-primary btn-sm\" for=\"news-image\">
                                        <input id=\"news-image-check-skip\" type='checkbox' checked hidden>
                                        <input id=\"news-image\" type=\"file\" style=\"display:none\"
                                        onchange=\"$('#upload-file-info').html(this.files[0].name);
                                                   $('#news-image-check-skip').prop('checked', false);
                                                   previewFile();\">
                                        Upload
                                    </label>
                                    <div>
                                ...
                                Content:
                                <textarea class=\"form-control\" style=\"min-width: 100%; min-height: 300px;\" id=\"news-content\">$content</textarea>
                                Created By: <input type=\"text\" class=\"form-control\" id=\"created-by\" value=\"$createdBy\" readonly>
                                Created Date: <input type=\"text\" class=\"form-control\" id=\"created-date\" value=\"$date\" readonly>
                                Views: <input type=\"text\" class=\"form-control\" id=\"news-views\" value=\"$views\" readonly>
            
                                <div style='padding-bottom: 5px; padding-top: 5px;'>
                                    <a data-toggle=\"modal\" data-target=\"#alert-modal\">
                                        <button type=\"button\" class=\"btn btn-success\" id=\"save-news\" data-news-id=\"$id\">Save</button>
                                    </a>
                                    <a data-toggle=\"modal\" data-target=\"#confirmation-modal\">
                                        <button type=\"button\" class=\"btn btn-danger\" id=\"delete-news\" data-news-id=\"$id\">Delete</button>
                                    </a>
                                </div>";
