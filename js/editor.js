	...
	var methods = {
		...
		imageWidget: function(){
			...
			handleFileSelect = function(evt) {
				...
				for (var i = 0, f; f = files[i]; i++) {
					...
					if(!f.type.match('postImage.*') || !f.name.match(/(?:gif|jpg|png|jpeg)$/)){ //Process only Images
			...
			var getURL = $("<button/>",{
				...
				if(url ==''){
					methods.showMessage.apply(this,["imgErrMsg","Please enter postImage url"]);
					return false;
				}
				...
				var image = $('<img/>',{
					src:url,
				  	...
				  	methods.showMessage.apply(this,["imgErrMsg","Invalid postImage url"]);
				  	return false;
						   ...
						   'insert_img'	: { "modal": true,
											...
											"onSave": function(){
												...
												if($('#imageList').data('current')){
													...
													methods.showMessage.apply(this,["imgErrMsg","Please select an postImage"]);
													return false;
												}
		...
		createImageContext: function(event,cMenuUl){
			...
			var onSave = function(){
				...
				if(imageAlt==""){
					methods.showMessage.apply(this,["imageErrMsg","Please enter postImage alternative text"]);
					return false;
				}
